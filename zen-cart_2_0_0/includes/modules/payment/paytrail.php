...
{
  return SignaturePyt::calculateHmac($params, $body, $this->secretKey);
}

public function validateHmac($response = [], $body = '', $signature = '')
{
  SignaturePyt::validateHmac($response, $body, $signature, $this->secretKey);
}
...
public function itemArgs($order)
{
 ...
 if (abs(isset($_SESSION['cc_id'])))
 {
  ...
  if ($redeemPoints > 0) 
  {
    $redem_value = number_format($redeem_points, 2, '.', '') * 100;
    // if tax is to be calculated on purchased GVs, calculate it
...
class SignaturePyt
{
  //public static function calculateHmac($secretKey, $params, $body = '')
  public static function calculateHmac($params = [], $body = '', $secretKey = '')
  {
